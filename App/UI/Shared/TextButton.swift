...
/// Text Button is a subclass of `UIButton` that implement an internal Bending Spoons standard.
/// This element is meant to be used as a button without background and only with a single, central text.
///
...
///
/// The usage is pretty straighforward, you just have to assign the `attributedTitle` property.
open class TextButton: UIButton {
  ...
  public func setTitleOpacity(_ opacity: CGFloat, for state: UIControl.State) {
  ...
  private func update() {
    self.setAttributedTitle(self.attributedTitle?.adapted(), for: .normal)
    self.titleLabel?.alpha = self.opacities[self.state.rawValue] ?? 1.0
  }
