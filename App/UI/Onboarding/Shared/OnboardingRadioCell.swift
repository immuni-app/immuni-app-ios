...
struct OnboardingRadioCellVM: ViewModel, Hashable {
  let title: String
  let isSelected: Bool
...
class OnboardingRadioCell: UICollectionViewCell, ModellableView, ReusableView {
  ...
  func update(oldModel: OnboardingRadioCellVM?) {
...
private extension OnboardingRadioCell {
  enum Style {
    static func checkmark(_ view: UIImageView, isSelected: Bool) {
      if isSelected {
        view.image = Asset.Onboarding.radioSelected.image
      } else {
        view.image = Asset.Onboarding.radio.image
      }
    }
